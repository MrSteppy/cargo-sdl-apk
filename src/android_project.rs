...
pub fn build_sdl_for_android(targets: &Vec<&str>, profile: BuildProfile) {
    ...
    for rust_name in targets {
        ...
        copy(&src, &dest).expect(&format!("Unable to copy SDL dependencies from {:?} to {:?}", src, dest));
    }
}
...
fn create_android_project(
    ...
    let app_icon = get_toml_string(manifest_path, vec!["package", "metadata", "android", "icon"]);
    ...
    if let Some(icon_path) = app_icon {
        ...
        for res in vec!["m", "h", "xh", "xxh", "xxxh"] {
            let dest = manifest_dir.join(format!("target/android-project/app/src/main/res/mipmap-{}dpi/ic_launcher.png", res));
            if let Err(e) = copy(&icon_path, &dest) {
                eprintln!("Failed to copy icon from {:?} to {:?}: {}", icon_path, dest, e);
            }
        }
    }
}
